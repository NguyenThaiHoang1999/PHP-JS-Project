...
<div class="">
    <ol class="breadcrumb">
        <li>
            <a
                href="<?php echo $sp->hrefdanhsachsanpham('dsSP', $_GET['loai'], $_GET['hang'], $_GET['sx'], $_GET['page'], $_GET['key']); ?>">Trang
                chủ</a>
        </li>
    ...
    <div class="row">
        ...
        <div class="col-md-2 text-center col-sm-4 col-xs-6" style="height: 300px; width: 250px;">
            <div class="thumbnail product-box" style="height: 290px; width: 240px;">
                <a style="height: 240px; width: 230px;"
                    href="<?php echo $sp->hrefdanhsachsanpham('ctSP', $_GET['loai'], $_GET['hang'], $_GET['sx'], $_GET['page'], $_GET['key']) . '&idSP=' . $rowSP['id_product']; ?>"
                    class="btn btn-default" role="button" title="<?php echo $rowSP['info_product']; ?>">
                    <?php if ($rowSP['sale_product'] >= $rowSP['price_product']): ?>
                    <span class="offer-text fa fa-bitcoin" style="padding: 5px 10px; border-radius: 3px;">
                        <?php echo $rowSP['price_product']; ?> Vnd</span>
                    <?php else: ?>
                    <span class="offer-text2 fa fa-tags" style="padding: 5px 10px; border-radius: 3px;">
                        <?php echo $rowSP['sale_product']; ?> Vnd</span>
                    <?php endif; ?>
                    <img src="public/images/<?php echo $sp->row_img($rowSP['image_product'])['url_image']; ?>"
                        alt="<?php echo $rowSP['name_product']; ?>" class="img-responsive" />
                </a>
                ...
                <a href="index?ac=addcart&sl=1&idSP=<?php echo $rowSP['id_product']; ?>&user=<?php if(isset($_COOKIE['account'])){echo $_COOKIE['account'];}else{echo "Loi";} ?>" class="btn btn-default"
                   name="addcart" title="Thêm vào giỏ hàng">+</a>
            </div>
        </div>
        <?php
            endforeach;
